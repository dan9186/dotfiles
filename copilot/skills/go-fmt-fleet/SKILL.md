---
name: go-fmt-fleet
description: 'Runs `go fmt ./...` across every Go repo in the current org directory, creates a branch and PR for each repo where formatting changed, and produces a summary table with repo names and PR links. Use when asked to "fmt all repos", "run go fmt across the org", "run go fmt for my org", "run go fmt for the org", "format all services", "go fmt everything", "format all my Go repos", "I want to run go fmt", "I''d like to run go fmt", "let''s run go fmt", or any phrasing combining "go fmt" with org-level scope. Skips repos where nothing changed — no branch, no commit, no PR.'
---

# Go Fmt Fleet

Run `go fmt ./...` across every Go repo in a multi-repo org directory, open a PR for each repo
where formatting changed, and produce a final table of results.

<TriggerPhrases>
- User asks to "run go fmt across the org" or "run go fmt for my org"
- User says "fmt all repos", "go fmt everything", or "format all services"
- User says "I'd like to run go fmt", "I want to run go fmt", or "let's run go fmt"
- Any request to apply `go fmt ./...` in bulk across a multi-repo root
</TriggerPhrases>

<Constraints>
- **Always goes to PR** — there is no stage-only mode for this skill.
- **Skip cleanly** — if `go fmt` produces no diff in a repo, do not create a branch, commit, or PR. Record it as "no changes" in the report.
- **Fixed PR title** — `chore: go fmt ./...`, identical across every repo in the batch.
...
- **No build/test/lint** — this is a formatting pass only. Do not run `go build`, `go test`, or any linter.
...
- Otherwise use `go-fmt` as the default. Announce the branch being used (e.g. "Using branch `go-fmt` — specify a different name if needed.").
...
- Absolute repo path
- Branch name to use
- PR title: `chore: go fmt ./...`
- PR body: `Automated formatting pass via \`go fmt ./...\`.`
...
10. Open the PR:
    ...
    gh pr create \
      --title "chore: go fmt ./..." \
      --body "Automated formatting pass via \`go fmt ./...\`." \
      --base <default-branch> \
      --head <branch>
    ```
...
- `pr-opened` — formatting changed; PR created.
...
If the table is empty (no PRs opened, no dirty repos), print: "All repos already formatted — nothing to do."
...
4. After confirmation, update `~/dotfiles/copilot/skills/go-fmt-fleet/SKILL.md`.
